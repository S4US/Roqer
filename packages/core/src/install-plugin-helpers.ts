...
// @roqer/mcp and @roqer/mcp-inspector. Bundled
...
/**
 * Whether a PluginsDir value is the live test harness's sentinel.
 *
 * The harness writes the bare relative name, but Studio does not keep it that
 * way: on its next settings save it resolves the name against its own working
 * directory and writes the result back, so a Studio launched from the Start
 * menu leaves `C:/WINDOWS/system32/RsmcpIsolatedPlugins` behind. Matching only
 * the bare name meant the repair recognised the sentinel exactly until Studio
 * had touched it, which is the case that needs repairing.
 */
function isStaleTestPluginsDirectory(value: string): boolean {
  const segments = value.trim().split(/[\\/]+/u);
  return segments[segments.length - 1] === STALE_TEST_PLUGINS_DIRECTORY;
}

/**
 * Repairs the PluginsDir sentinel used by the live test harness, bare or as
 * Studio resolved it. Other custom Studio plugin directories are user-owned and
 * remain untouched.
 */
export function repairStaleStudioPluginDirectorySetting({
  ...
  if (!isStaleTestPluginsDirectory(matches[0][2])) return false;
