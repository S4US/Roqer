...
export interface ServerConfig {
  ...
  /**
   * The Studio plugin build this server pairs with. The inspector edition's
   * read-only promise holds only if an inspector server never talks to a full
   * plugin's session through another edition, so the HTTP bridge refuses a
