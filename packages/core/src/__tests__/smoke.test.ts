...
/**
 * Tools whose background registry work a test has started. Each test waits for
 * its own before the next begins: otherwise an association left running into a
 * later test that installs fake timers is frozen holding the registry lock, and
 * the rest time out after the file has finished.
 */
const liveTools: RobloxStudioTools[] = [];

function trackedTools(bridge: BridgeService): RobloxStudioTools {
...
afterEach(async () => {
  await Promise.all(liveTools.splice(0).map((tools) => tools.settled()));
});
...
describe('Smoke', () => {
  ...
  test('manage_instance threads the working directory into launch and refuses a caller-chosen executable or environment', async () => {
    const bridge = new BridgeService();
    const tools = trackedTools(bridge);
    const launch = jest.fn(async (options) => ({
      ...options,
    ...
    expect(options).toEqual(expect.objectContaining({ studioWorkingDirectory: 'C:\\Studio Workers\\worker-7' }));
    expect(options.studioExecutable).toBeUndefined();
    expect(options.processEnvironment).toBeUndefined();

    // The program and its environment come from where the server runs. A
    // caller naming either is refused before anything launches.
    await expect(tools.manageInstance({
      action: 'launch',
      source: 'local_file',
      local_place_file: '/tmp/custom-launch-place.rbxl',
      studio_executable: 'C:\\Windows\\System32\\cmd.exe',
    })).rejects.toThrow(/ROBLOX_STUDIO_EXE/);
    await expect(tools.manageInstance({
      action: 'launch',
      source: 'local_file',
      local_place_file: '/tmp/custom-launch-place.rbxl',
      process_environment: { set: { STUDIO_LAUNCH_LOADER: 'C:\\LaunchTools\\studio_loader.dll' } },
    })).rejects.toThrow(/does not take process_environment/);
    expect(launch).toHaveBeenCalledTimes(1);
  });
  ...
  test('identity-required launch retains ownership until explicitly completed', async () => {
    ...
    try {
      ...
      const tools = trackedTools(new BridgeService());
  ...
  test('manage_instance asynchronously associates and closes a no-wait local-file launch', async () => {
    ...
    const stopped: number[] = [];
    const localPlaceFile = '/home/dev/projects/my-game/place.rbxl';
    const processAdapter = {
      currentBootId: () => 'boot-1',
      resolveStudioExe: () => 'RobloxStudioBeta.exe',
      spawnStudio: () => {
  ...
  test('generate_model source image upload uses backing textureId instead of Decal wrapper id', async () => {
    const bridge = new BridgeService();
    const tools = trackedTools(bridge) as any;
  ...
  test('get_script_source returns structured truncation metadata', async () => {
    ...
    const resultPromise = tools.getScriptSource('game.ServerScriptService.Manager', undefined, undefined, 'place:test', 'instance:test:1');
    const pending = claimQueuedRequest(bridge, 'session-1');
    expect(pending?.request.endpoint).toBe('/api/get-script-source');
    expect(pending?.request.data).toMatchObject({
    ...
    bridge.resolveRequest(pending!.requestId, {
      ...
      instancePath: 'game.ServerScriptService.Manager',
      instanceRef: 'instance:test:1',
      revision: 'sr1:123:0011223344556677',
      className: 'Script',
  ...
  test('set_script_source routes a stable reference and expected revision to the selected Studio instance', async () => {
    const bridge = new BridgeService();
    const tools = trackedTools(bridge);
    bridge.registerInstance(READY);

    const resultPromise = tools.setScriptSource(
      'game.ServerScriptService.Manager',
      'return true',
      'place:test',
      'sr1:10:0011223344556677',
      'instance:test:1',
    );
    const pending = claimQueuedRequest(bridge, 'session-1');
    expect(pending?.request.endpoint).toBe('/api/set-script-source');
    expect(pending?.request.data).toEqual({
      instancePath: 'game.ServerScriptService.Manager',
      instanceRef: 'instance:test:1',
      source: 'return true',
      expectedRevision: 'sr1:10:0011223344556677',
    });
    bridge.resolveRequest(pending!.requestId, { success: true });
