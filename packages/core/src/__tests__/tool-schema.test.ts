...
describe('Tool schema compatibility', () => {
  ...
  test('selection exposes one get/set/open/view lifecycle', () => {
    const tool = TOOL_DEFINITIONS.find(candidate => candidate.name === 'selection');
    expect(tool).toBeDefined();
    expect(tool!.category).toBe('read');
    expect(getReadOnlyTools()).toContain(tool);
    expect(tool!.description).toBe('Use to get, set, open, or frame Studio context.');
    ...
    expect(props.action.enum).toEqual(['get', 'set', 'open', 'view']);
    expect(props.mode).toMatchObject({ enum: ['set', 'add', 'remove'], default: 'set' });
    expect(props.action.description).toBe('Open shows a script; view frames a 3D target.');
  ...
  test('get_script_source exposes only line_range for range selection', () => {
    ...
    expect(Object.keys(props).sort()).toEqual(['instancePath', 'instanceRef', 'instance_id', 'line_range']);
  ...
  test('safe mutation fields extend existing tools without adding a public tool', () => {
    const setProperties = TOOL_DEFINITIONS.find(tool => tool.name === 'set_properties')!;
    const setPropertiesProps = (setProperties.inputSchema as { properties?: Record<string, unknown> }).properties ?? {};
    expect(setPropertiesProps).toHaveProperty('instanceRef');

    const setSource = TOOL_DEFINITIONS.find(tool => tool.name === 'set_script_source')!;
    const setSourceSchema = setSource.inputSchema as { properties?: Record<string, unknown>; required?: string[] };
    expect(setSourceSchema.properties).toHaveProperty('instanceRef');
    expect(setSourceSchema.properties).toHaveProperty('expectedRevision');
    expect(setSourceSchema.required).toEqual(['instancePath', 'source', 'expectedRevision']);

    expect(TOOL_DEFINITIONS.some(tool => tool.name === 'get_instance_reference')).toBe(false);
  });

  test('inspect_ui exposes one bounded runtime semantic inspection surface', () => {
    const tool = TOOL_DEFINITIONS.find(candidate => candidate.name === 'inspect_ui');
    expect(tool).toBeDefined();
    expect(tool!.category).toBe('read');
    const schema = tool!.inputSchema as {
      properties?: Record<string, {
    ...
    expect(props.mode.enum).toEqual(['inspect', 'audit', 'snapshot']);
    expect(props.max_depth).toMatchObject({ minimum: 0, maximum: 32 });
    expect(props.max_nodes).toMatchObject({ minimum: 1, maximum: 1000 });
    expect(Object.keys(props.root.properties ?? {}).sort()).toEqual(['path', 'ref']);
  });

  test('interact_ui exposes one selector-based runtime interaction surface', () => {
    const tool = TOOL_DEFINITIONS.find(candidate => candidate.name === 'interact_ui');
    expect(tool).toBeDefined();
    ...
    const props = schema.properties ?? {};
    expect(schema.required).toEqual(['action', 'selector']);
    expect(Object.keys(props).sort()).toEqual(['action', 'canvas_position', 'instance_id', 'selector', 'target', 'text']);
    expect(props.action.enum).toEqual(['click', 'focus', 'type', 'scroll_into_view', 'set_scroll']);
    expect(props.selector.minProperties).toBe(1);
    expect(Object.keys(props.selector.properties ?? {}).sort()).toEqual([
      'ancestor', 'class', 'name', 'path', 'ref', 'role', 'text', 'visible_only',
    ]);
    expect(props.canvas_position.required).toEqual(['x', 'y']);
  });
  ...
  test('upload_asset keeps legacy uploads valid and exposes durable status checks', async () => {
    const schema = TOOL_DEFINITIONS.find(tool => tool.name === 'upload_asset')!.inputSchema as {
    ...
    const uploadAsset = jest.fn(async () => ({ content: [] }));
    await TOOL_HANDLERS.upload_asset({ uploadAsset } as unknown as RobloxStudioTools, {
      action: 'status',
      operationId: 'upload-123',
    });
    expect(uploadAsset).toHaveBeenCalledWith(
      undefined, undefined, undefined, undefined, undefined, undefined,
      'status', 'upload-123', undefined,
    );
  });
  ...
  test('every Studio-routing tool implementation accepts an instance_id parameter', () => {
    ...
    const methodNameOf: Record<string, string> = {
      ...
      edit_script_batch: 'editScriptBatch',
  ...
  test('manage_instance exposes launch authorization, lifecycle actions, and place version discovery in one schema', () => {
    ...
    // The executable and its environment are the server's to set, not a caller's.
    expect(props.studio_executable).toBeUndefined();
    expect(props.process_environment).toBeUndefined();
