...
export class OpenCloudClient {
  ...
  /** Read a previously returned upload operation without starting another upload. */
  async getAssetOperation(operationId: string): Promise<AssetOperationResponse> {
    const normalized = operationId.startsWith('operations/')
      ? operationId.slice('operations/'.length)
      : operationId;
    if (!/^[A-Za-z0-9_-]{1,256}$/.test(normalized)) {
      throw new Error('Asset upload operation ID is not valid.');
    }
    return this.request<AssetOperationResponse>(
      `/assets/v1/operations/${encodeURIComponent(normalized)}`
    );
  }
  ...
  private async pollOperation(
    ...
    let latest: AssetOperationResponse = { path: operationPath, done: false };
    for (let i = 0; i < maxAttempts; i++) {
      const result = await this.getAssetOperation(operationId);
      latest = result;
      if (result.done || result.error) return result;
      await new Promise(resolve => setTimeout(resolve, intervalMs));
    }
    // Roblox keeps operations addressable after this bounded initial wait. A
    // pending result is useful: the caller can check it later without
    // re-uploading the file (and without risking a duplicate asset).
    return latest;
  }
}
