...
| `smart_duplicate`, `mass_duplicate` | `build_instances` `clone` steps, one transform per copy; `execute_luau` for patterns computed from existing geometry |
...
| `create_object`, `mass_create_objects`, `delete_object`, `clone_object` | `build_instances` `create`, `clone`, and `remove` steps under one build root, applied atomically as one undo step |
| `set_attribute`, `delete_attribute`, `bulk_set_attributes` | Set attributes with `build_instances` `attributes` inside a build root; clear them with `execute_luau`; `get_attributes` remains for compact reads |
| `get_tags`, `add_tag`, `remove_tag`, `get_tagged` | Add tags with `build_instances` `tags`; read or remove them with `CollectionService` through `execute_luau` |
...
## Removed `manage_instance` parameters

`manage_instance` no longer takes `studio_executable` or `process_environment`.
Together they let whoever called the tool, usually a model, start any program
on the machine with any environment. A call that passes either is refused with
an error naming the replacement:

| Removed parameter | Replacement |
| --- | --- |
| `studio_executable` | Set `ROBLOX_STUDIO_EXE` where the MCP server runs; see [configuration](configuration.md). |
| `process_environment` | Studio inherits the MCP server's environment, so set variables there, for example in the MCP client's server configuration. |

`studio_working_directory` remains.
