...
-- THEME BUTTONS: the canonical studded bevel-stack button. Paste as written.
-- Every button below is created through it; a hand-built button is a defect.
local STUD_PATTERN = "rbxassetid://92521981645530"
local ROBUX_ICON = "rbxassetid://87608142780557"
local BUTTON_COLORS = {
	green = { base = Color3.fromRGB(6, 84, 12), tint = Color3.fromRGB(190, 255, 190), main = Color3.fromRGB(34, 197, 63) },
	gold = { base = Color3.fromRGB(105, 70, 0), tint = Color3.fromRGB(255, 233, 170), main = Color3.fromRGB(255, 176, 16) },
	red = { base = Color3.fromRGB(120, 14, 14), tint = Color3.fromRGB(255, 130, 130), main = Color3.fromRGB(239, 28, 28) },
}

local function makeStudButton(parent, colorName, labelText, size)
	local colors = BUTTON_COLORS[colorName]
	local button = Instance.new("TextButton")
	button.Text = ""
	button.BackgroundColor3 = colors.base
	...
	face.Position = UDim2.new(0.5, 0, 0, 0)
	...
	pattern.Parent = face
	task.defer(function()
		local tile = math.max(8, math.round(button.AbsoluteSize.Y * 0.9))
...
local closeBtn = makeStudButton(wheelFrame, "red", "X", UDim2.new(0.03, 0, 0.053, 0))
...
local spinBtn = makeStudButton(wheelFrame, "gold", "SPIN", UDim2.new(0.072, 0, 0.037, 0))
...
for i, pack in ipairs(CONFIG.buyPacks) do
	local btn = makeStudButton(wheelFrame, "green", "", UDim2.new(0.129, 0, 0.054, 0))
...
openTimerStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
openTimerStroke.Parent = openTimer
local openTimerText = makeLabel(openTimer, CONFIG.freeSpinTimer, 0.5, 0.5, 0.8, 0.8, 5)
