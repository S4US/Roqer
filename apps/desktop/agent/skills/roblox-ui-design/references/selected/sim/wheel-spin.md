# Recovered selected body: SIM / wheel-spin
...
The wheel chrome keeps its exact assets in every theme. The BUTTONS (open, close, SPIN, buy packs) are NOT chrome: the deployed template already builds every one of them with the style guide's `makeStickerButton` (green for the Robux buy packs, blue-cyan for SPIN, red for the close X) and already carries the catalog Robux icon beside the buy-pack prices. Wedge accent colors come from CONFIG and should sit in the theme's bright candy range.
...
- Every button (open, close, SPIN, buys) is a `makeStickerButton` button. Reusing the source system's pill/badge button images is a defect.
