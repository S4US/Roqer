# Recovered selected body: STUDS / vertical-reel-roll
...
The reel itself is theme-neutral: no panels, identical in both themes; the item art is the content surface (rbxthumb for real catalog items, icons-and-images catalog art for game items). The ROLL button is the one themed element: the deployed template already builds it as the canonical studded bevel-stack button (dark base + black stroke 4, face 0.90 with the 3-stop shine, edge-to-edge studs), green, with the dice icon beside the label.
