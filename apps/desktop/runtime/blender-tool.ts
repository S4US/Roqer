...
export function blenderToolDefinition(): Readonly<{ name: typeof BLENDER_TOOL_NAME; description: string; inputSchema: JsonRecord }> {
  return {
    ...
    description: [
      ...
      "Roqer does not trust the script's own report: it re-imports each exported .glb/.gltf/.fbx/.obj, returns its triangles, meshes, materials, size and where its colour lives, and attaches a preview render; a PNG is attached as itself with its pixel size. Look at them before using them.",
