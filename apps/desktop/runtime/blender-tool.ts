...
export function blenderToolDefinition(): Readonly<{ name: typeof BLENDER_TOOL_NAME; description: string; inputSchema: JsonRecord }> {
  return {
    ...
    description: [
      ...
      "The script runs in background Blender on an empty scene with bpy imported and OUTPUT_DIR defined. Build with bpy, then export into OUTPUT_DIR, for example bpy.ops.export_scene.gltf(filepath=os.path.join(OUTPUT_DIR, 'crate.glb'), export_format='GLB', export_apply=True), or render a PNG there. Write nowhere else and fetch nothing from the network.",
      "Roqer does not trust the script's own report: it re-imports each exported .glb/.gltf/.fbx/.obj, returns its triangles, meshes, materials, size, where its colour lives, and its layout (pieces that float or pass into each other), and attaches four views of it in one preview; a PNG is attached as itself with its pixel size. Look at them before using them.",
    ...
    inputSchema: {
      ...
      properties: {
        script: {
          ...
          description: "The complete Python script. bpy and OUTPUT_DIR are already defined.",
        },
        timeout_seconds: {
          type: "number",
          minimum: 5,
