...
export function blenderToolDefinition(): Readonly<{ name: typeof BLENDER_TOOL_NAME; description: string; inputSchema: JsonRecord }> {
  return {
    ...
    description: [
      ...
      `A script may be at most ${MAX_BLENDER_SCRIPT_CHARACTERS.toLocaleString("en-US")} characters: build repeated parts (wheels, bolts, tubes, panels) with loops and functions rather than writing each out, and model an object too detailed for one script across several jobs that each export their own parts, placed together in Studio.`,
