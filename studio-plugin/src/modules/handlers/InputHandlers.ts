...
function simulateMouseInput(requestData: Record<string, unknown>) {
	...
	const [success, err] = pcall(() => {
		if (action === "click") {
			...
			// MouseButton1Click is dispatched after the release call yields back to
			// the engine. Do not report a completed click while its target callback
			// can still be invalidated by the caller's immediate follow-up action.
			task.wait(0.1);
