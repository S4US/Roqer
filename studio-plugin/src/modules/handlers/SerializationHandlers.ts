...
/**
 * Collect a payload the request was too large to carry.
 *
 * A request reaches this plugin as one SSE line, and a large one never arrives
 * at all -- the stream client drops it and the handler is simply never called.
 * So anything sizeable is left on the bridge and fetched here, over the same
 * ordinary HTTP that already carries a large export back.
 */
function fetchImportPayload(payloadId: string): { base64?: string; error?: string } {
	const url = `${ClientBroker.getServerUrl()}/import-payload/${payloadId}`;
	const [requestOk, result] = pcall(() =>
		HttpService.RequestAsync({ Url: url, Method: "GET" }),
	);
	if (!requestOk) return { error: `failed to collect import payload: ${tostring(result)}` };
	const response = result as RequestAsyncResponse;
	if (!response.Success) {
		return { error: `import payload request failed with HTTP ${response.StatusCode}` };
	}
	const [decodeOk, decoded] = pcall(() => HttpService.JSONDecode(response.Body));
	if (!decodeOk || !typeIs(decoded, "table")) {
		return { error: "import payload response was not readable" };
	}
	const base64 = (decoded as Record<string, unknown>).base64;
	if (!typeIs(base64, "string") || base64 === "") {
		return { error: "import payload response carried no bytes" };
	}
	return { base64 };
}

function importRbxm(requestData: Record<string, unknown>): unknown {
	let b64 = requestData.base64 as string | undefined;
	const payloadId = requestData.payload_id as string | undefined;
	...
	if ((!b64 || !typeIs(b64, "string")) && typeIs(payloadId, "string") && payloadId !== "") {
		const collected = fetchImportPayload(payloadId);
		if (collected.error !== undefined) return { error: collected.error };
		b64 = collected.base64;
	}
