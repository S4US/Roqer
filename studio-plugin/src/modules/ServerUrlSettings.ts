...
/**
 * A remembered `http://localhost:<port>` becomes `http://127.0.0.1:<port>`.
 *
 * Every install that ever connected remembered the old default, so changing
 * the default alone would have left existing users on the spelling that hangs
 * on some machines. Only the bare loopback host is rewritten: an address a
 * person typed for a bridge somewhere else is theirs.
 */
function preferLoopbackAddress(serverUrl: string): string {
	const [port] = serverUrl.match("^http://localhost:(%d+)$");
	return port === undefined ? serverUrl : `http://${State.DEFAULT_HOST}:${port}`;
}
...
function readSettingString(key: string): string | undefined {
	...
	return normalized !== "" ? preferLoopbackAddress(normalized) : undefined;
}
