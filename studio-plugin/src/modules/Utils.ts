import InstanceReferences from "./InstanceReferences";

const ScriptEditorService = game.GetService("ScriptEditorService");

const { getInstanceReference, resolveInstanceReference } = InstanceReferences;
...
const COLOR_COMPONENT_EPSILON = 1 / 255 + 1e-6;

function sameNumber(actual: number, requested: number): boolean {
	return math.abs(actual - requested) <= 1e-6 * math.max(1, math.abs(requested));
}

/**
 * Whether Studio read a property back as the value that was requested.
 *
 * Numeric Roblox value types are float-backed, while Color3-backed properties
 * such as BasePart.Color may quantize a requested channel downward by one full
 * 8-bit step (for example 0.2627 -> 66/255). Verification therefore compares
 * representable values rather than Lua userdata identity.
 */
function samePropertyValue(actual: unknown, requested: unknown): boolean {
	if (typeIs(actual, "number") && typeIs(requested, "number")) {
		return sameNumber(actual, requested);
	}

	const actualType = typeOf(actual);
	if (actualType !== typeOf(requested)) return false;

	if (actualType === "Color3") {
		const a = actual as Color3;
		const b = requested as Color3;
		return math.abs(a.R - b.R) <= COLOR_COMPONENT_EPSILON &&
			math.abs(a.G - b.G) <= COLOR_COMPONENT_EPSILON &&
			math.abs(a.B - b.B) <= COLOR_COMPONENT_EPSILON;
	}
	if (actualType === "Vector3") {
		const a = actual as Vector3;
		const b = requested as Vector3;
		return sameNumber(a.X, b.X) && sameNumber(a.Y, b.Y) && sameNumber(a.Z, b.Z);
	}
	if (actualType === "Vector2") {
		const a = actual as Vector2;
		const b = requested as Vector2;
		return sameNumber(a.X, b.X) && sameNumber(a.Y, b.Y);
	}
	if (actualType === "UDim") {
		const a = actual as UDim;
		const b = requested as UDim;
		return sameNumber(a.Scale, b.Scale) && a.Offset === b.Offset;
	}
	if (actualType === "UDim2") {
		const a = actual as UDim2;
		const b = requested as UDim2;
...
/**
 * Walk one segment, honouring `Players.LocalPlayer`.
 *
 * `LocalPlayer` is a property of `Players`, not a child of it, so a walk built
 * on `FindFirstChild` can never reach it -- and `game.Players.LocalPlayer.
 * PlayerGui.<name>` is the ordinary way to name a live UI, the one every Roblox
 * example writes. An agent that asks for it correctly was being told the path
 * does not exist, which is both wrong and unactionable: the child it would have
 * had to name instead is the player's own name, which nothing had told it.
 *
 * Resolved here rather than in one handler because every path-taking operation
 * has the same problem. Outside a running client `LocalPlayer` is nil and the
 * walk falls through to `FindFirstChild`, which is what it did before, so an
 * edit-mode caller still gets nothing -- and an instance genuinely named
 * "LocalPlayer" is still found.
 */
function resolvePathSegment(parent: Instance, segment: string): Instance | undefined {
	if (segment === "LocalPlayer" && parent.ClassName === "Players") {
		const [ok, player] = pcall(() => (parent as Players).LocalPlayer);
		if (ok && player) return player;
	}
...
/**
 * The live parent a path names and the final segment to create beneath it, for
 * an operation that makes the path exist rather than finding it. Returns
 * nothing for `game` or a bare service name: a service cannot be created.
 */
function resolveParentAndName(path: string): { parent?: Instance; name?: string } {
	const parts = parseInstancePath(path);
	if (parts === undefined || parts.size() < 2) return {};
	let parent: Instance | undefined = getRootInstance(parts[0]);
	for (let i = 1; i < parts.size() - 1; i++) {
		if (!parent) break;
		parent = resolvePathSegment(parent, parts[i]);
	}
	return { parent, name: parts[parts.size() - 1] };
}

function resolveInstance(instancePath?: string, instanceRef?: string): Instance | undefined {
	if (instanceRef !== undefined) {
		return resolveInstanceReference(instanceRef);
	}
	if (instancePath === undefined || instancePath === "") return undefined;
	return getInstanceByPath(instancePath);
}
...
/**
 * A Color3 from 0-1 components. Color3.new takes 0-255 values without complaint
 * and stores a colour nothing asked for: a model that sent [107, 64, 31] for
 * brown got pale blue and only found out from a screenshot. Refusing says why.
 */
function color3From(r: unknown, g: unknown, b: unknown): Color3 {
	const components = [r ?? 0, g ?? 0, b ?? 0];
	for (const component of components) {
		if (!typeIs(component, "number") || component < 0 || component > 1 + 1e-6) {
			error(
				`Color3 components are numbers from 0 to 1, got [${components.map((value) => tostring(value)).join(", ")}]; ` +
					"divide 0-255 values by 255",
			);
		}
	}
...
function convertPropertyValue(instance: Instance, propertyName: string, propertyValue: unknown): unknown {
	...
	if (typeIs(propertyValue, "table")) {
		...
		if (typeIs(arr, "table") && (arr as defined[]).size() > 0) {
			...
			if (len === 3) {
				...
				if (
					...
					return color3From(arr[0], arr[1], arr[2]);
				} else {
		...
		if (tbl.R !== undefined || tbl.G !== undefined || tbl.B !== undefined) {
			return color3From(tbl.R, tbl.G, tbl.B);
		}
	}
