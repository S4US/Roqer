...
// An empty id means no published icon yet. Studio then draws the button as its
// title text, whereas "rbxassetid://" with nothing after it is a broken image.
function iconContent(assetId: string): string {
	return assetId === "" ? "" : `rbxassetid://${assetId}`;
}

const ICON_DISCONNECTED = iconContent("__BUTTON_ICON_DISCONNECTED__");
const ICON_CONNECTING = iconContent("__BUTTON_ICON_CONNECTING__");
const ICON_CONNECTED = iconContent("__BUTTON_ICON_CONNECTED__");
...
// Auto-activate per peer because the plugin UI is not available in play
// DataModels. Run after a short delay so UI and state initialization finish.
task.delay(2, () => {
